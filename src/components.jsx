...
function Navbar({ admin }) {
  const {user,logout}=useAuth(); const navigate=useNavigate(); const [busy,setBusy]=useState(false); const [open,setOpen]=useState(false)
  ...
  useEffect(()=>{const button=document.querySelector('.app-navbar .navbar-toggler'); const menu=document.getElementById('appNav'); if(!button||!menu)return; button.removeAttribute('data-bs-toggle'); button.removeAttribute('data-bs-target'); const toggle=()=>{setOpen(value=>{const next=!value; menu.classList.toggle('show',next); button.setAttribute('aria-expanded',String(next)); return next})}; button.addEventListener('click',toggle); return()=>button.removeEventListener('click',toggle)},[])
