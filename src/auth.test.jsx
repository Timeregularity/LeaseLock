...
function Probe() {
  ...
  return <span>{loading ? 'loading' : user?.role || 'guest'}</span>;
}
afterEach(() => vi.restoreAllMocks());

describe('authentication integration', () => {
  test('restores a database session on application load', async () => {
    vi.stubGlobal(
      'fetch',
      vi
        .fn()
  ...
  test('redirects a guest away from a protected route', async () => {
    vi.stubGlobal(
      'fetch',
      vi
        .fn()
