...
export async function runOutboxDelivery() {
  ...
  try {
    ...
    const pending = await client.query(
      'SELECT * FROM event_outbox WHERE published_at IS NULL ORDER BY id LIMIT 100 FOR UPDATE SKIP LOCKED',
    );
    for (const event of pending.rows) {
      try {
        if (event.topic === 'seats-changed')
          broadcastSeatChange(event.aggregate_id, event.payload.reason);
        await client.query(
