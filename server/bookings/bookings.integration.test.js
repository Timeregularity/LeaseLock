...
test('failed mock payment can be retried successfully',async()=>{const create=async scenario=>{const response=await fetch(`${baseUrl}/v1/payments`,{method:'POST',headers:jsonHeaders(),body:JSON.stringify({bookingId,method:'TEST_UPI',scenario})});return{response,body:await response.json()}}
