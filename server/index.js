...
const expiryTimer = setInterval(
  () => runHoldExpiry().catch((error) => console.error('Hold expiry job failed', error)),
  15_000,
);
expiryTimer.unref();
const waitlistTimer = setInterval(
  () => runWaitlistPromotion().catch((error) => console.error('Waitlist promotion failed', error)),
  15_000,
);
waitlistTimer.unref();
const cleanupTimer = setInterval(
  () => runCleanup().catch((error) => console.error('Cleanup job failed', error)),
  60 * 60 * 1000,
);
cleanupTimer.unref();
const outboxTimer = setInterval(
  () => runOutboxDelivery().catch((error) => console.error('Outbox delivery failed', error)),
  1_000,
);
outboxTimer.unref();
